...
void InspectorMode::refreshInspector() {
	...
	camera->position += movement_direction * movementSpeed * Runtime::delta_time;

	if (!Runtime::showEngineUI) {

		glm::vec3 rotate_direction = glm::vec3(-Input::mouseAxis.y, Input::mouseAxis.x, 0.0f);
		glm::vec3 new_rotation = camera->rotation + (rotate_direction * sensitivity * Runtime::delta_time);
