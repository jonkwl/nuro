...
void main()
{
    ...
    // Use model matrix to apply transforms on normals
    v_normals = mat3(transpose(inverse(model))) * normals_in;

    // Use model matrix to calculate fragments position
    v_fragmentPosition = vec3(model * vec4(position_in, 1.0));

    // Use light space matrix to calculate fragments position in light space
